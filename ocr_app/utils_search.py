...
sbert_model = SentenceTransformer("sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2", device="cuda" if torch.cuda.is_available() else "cpu")
...
def index_documents():
    """Indexes all documents using optimized FAISS and multilingual SBERT model."""
    ...
    # Encode documents with optimized batch processing
    batch_size = 64  # Increased batch size for better GPU utilization
    all_embeddings = []
    
    for i in range(0, len(texts), batch_size):
        batch = texts[i:i + batch_size]
        # Normalize text length for better batch processing
        batch = [text[:2000] for text in batch]  # Ensure consistent length
        with torch.no_grad():  # Disable gradient calculation for inference
            embeddings = sbert_model.encode(
                ...
                convert_to_tensor=False  # Ensure numpy output for FAISS
            )
...
def suggest_documents(query, top_n=5):
    ...
    if cache_key in embedding_cache:
        ...
        # Normalize query length for consistent embedding
        query = query[:2000]  # Match document length limit
        with torch.no_grad():  # Disable gradient calculation for inference
    ...
    # Get more candidates than needed for better quality
    k = min(top_n * 2, index.ntotal)
    distances, indices = index.search(query_embedding, k)
    
    # Filter results based on distance threshold
    distance_threshold = 0.8  # Adjust based on your needs
    suggested_docs = []
    for i, (dist, idx) in enumerate(zip(distances[0], indices[0])):
        if idx != -1 and idx < len(documents_db) and dist < distance_threshold:
            try:
                doc = Document.objects.get(id=documents_db[idx])
                suggested_docs.append(doc)
                if len(suggested_docs) >= top_n:
                    break
